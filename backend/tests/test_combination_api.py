...
def test_import_validation_persistence_latest_and_isolation() -> None:
    ...
    assert (
        client.post(
            "/api/v1/melate/imports", json={"csv_text": mel, "commit": True}
        ).json()["accepted_rows"]
        == 1
    )
    assert (
    ...
    assert (
        client.get("/api/v1/melate_retro/draws/latest").json()["game_slug"]
        == "melate_retro"
    )
    melate_rows = client.get("/api/v1/melate/draws").json()
    retro_rows = client.get("/api/v1/melate_retro/draws").json()
    assert (
        next(row for row in melate_rows if row["draw_number"] == "900001")[
            "natural_numbers"
        ][-1]
        == 56
    )
    assert (
        next(row for row in retro_rows if row["draw_number"] == "900001")[
            "natural_numbers"
        ][-1]
        == 39
    )
...
def test_statistics_generation_coverage_and_errors() -> None:
    ...
    assert (
        first.status_code == 200 and first.json()["tickets"] == second.json()["tickets"]
    )
...
def test_backtest_contract_and_cross_game_keys() -> None:
    for slug, prefix in (("melate", HEADER), ("melate_retro", RETRO)):
        rows = "".join(
            f"40,{910000+i},1,2,3,4,5,6,7,0,{i+1:02d}/02/2020\n" for i in range(8)
        )
        client.post(
            f"/api/v1/{slug}/imports", json={"csv_text": prefix + rows, "commit": True}
        )
        made = client.post(
        ...
        assert (
            client.get(f"/api/v1/{slug}/backtests/{made.json()['id']}").status_code
            == 200
        )
